...
export function ProfilePage() {
  ...
  const initials = (profile?.displayName || name || 'Musicista').split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()
  return (
      <main className="page form-page profile-page app-screen">
        <header className="profile-header"><motion.div className="profile-avatar" layoutId="profile-avatar" aria-hidden="true">{initials}</motion.div><h1>Profilo</h1></header>
        <section className="profile-section profile-identity-section">
          <div className="profile-section-heading"><h2>Nome e strumenti</h2></div>
          <form onSubmit={submit}>
            ...
            <fieldset className="instrument-picker"><legend>Quali strumenti suoni?</legend>
              {INSTRUMENTS.map((instrument) => <button type="button" key={instrument} className={instruments.includes(instrument) ? 'active' : ''} onClick={() => toggle(instrument)}><span>{instrument}</span>{instruments.includes(instrument) && <Check size={17} />}</button>)}
            </fieldset>
        ...
        {mode === 'supabase' && <section className="profile-section profile-password-section">
          <div className="profile-section-heading"><h2>Password</h2></div>
        ...
        {mode === 'supabase' && <section className="profile-session-section"><strong>Sessione</strong><button className="logout-button" type="button" onClick={() => { void supabase?.auth.signOut() }}><LogOut size={17} /> Esci</button></section>}
