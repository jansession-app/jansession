  ...
  return (
    <main className="page tab-page app-screen">
      <motion.header className="tab-header jam-section-header" layoutId={`jam-section-${jamId}-setlist`} transition={{ type: 'spring', stiffness: 370, damping: 34 }}>
        ...
        <div className="section-title-row"><h1>Scaletta</h1><span>{items.length}</span></div>
      </motion.header>
      {items.length ? (
            ...
            return (
              ...
              >
                ...
                {manager && <div className="order-controls">
                  <motion.button whileTap={reduceMotion ? undefined : { scale: 0.88, y: -1 }} onClick={() => actions.moveSetlist(song.id, -1)} disabled={index === 0} aria-label={`Sposta ${song.title} su`}><ArrowUp size={16} /></motion.button>
                  <motion.button whileTap={reduceMotion ? undefined : { scale: 0.88, y: 1 }} onClick={() => actions.moveSetlist(song.id, 1)} disabled={index === items.length - 1} aria-label={`Sposta ${song.title} giù`}><ArrowDown size={16} /></motion.button>
                  <motion.button whileTap={reduceMotion ? undefined : { scale: 0.88 }} onClick={() => actions.removeFromSetlist(song.id)} aria-label={`Rimuovi ${song.title} dalla scaletta`}><Trash2 size={16} /></motion.button>
                </div>}
              </motion.li>
            )
          })}</AnimatePresence>
        </motion.ol>
      ) : <EmptyState icon={ListMusic} title="Scaletta vuota" body="Aggiungi un brano pronto." />}

      {manager && available.length > 0 && <section className="section-block add-setlist">
        <div className="section-heading"><div><h2>Aggiungi</h2></div></div>
